...
\begin{figure}[H]
  ...
  Źródło: IBM Developer Works\footnote{\url{http://www.ibm.com/developerworks/library/wa-reverseajax2/}}
\end{figure}
...
\begin{figure}[H]
  ...
  Źródło: Pro HTML5 Programming, Using the WebSocket API\footnote{\url{http://apress.jensimmons.com/v5/pro-html5-programming/ch7.html}}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption[Varnish cache miss]{Varnish cache miss}
    Rysunek przedstawia przypadek, gdy Varnish nie znalazł wpisu w cache oraz przekazuje żądanie do serwera aplikacji, aby otrzymać odpowiedź. Źródło \cite{varnish-cache}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption[Varnish cache hit]{Varnish cache hit}
    Rysunek przedstawia przypadek, gdy Varnish znalazł wpis w cache, nie jest konieczne przekazanie połączenia do serwera aplikacji, odpowiedź jest wczytywana prosto z pamięci RAM. Źródło \cite{varnish-cache}
\end{figure}
